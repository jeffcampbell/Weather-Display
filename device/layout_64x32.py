...
# ---------------------------------------------------------------------------
# Feature flags (per-device, from secrets.py). Each capability can be turned
# off independently. Defaults are all-on, matching this layout's original
# always-enabled behavior. `enable_astronomy` is accepted for config
# compatibility with the 128x64 layout but has no effect here — astronomy
# (sky map / planets / forecast) is a 128x64-only feature.
# ---------------------------------------------------------------------------
def _flag(name, default):
    return bool(secrets.get(name, default))

ENABLE_WEATHER = _flag("enable_weather", True)
ENABLE_TIDE    = _flag("enable_tide", True)
ENABLE_PLANES  = _flag("enable_planes", True)
ENABLE_BOATS   = _flag("enable_boats", True)
...
# unreachable). One TileGrid per group because displayio doesn't
...
def set_route_free_tier(free):
    ...
    if route_label.text:
        route_label.color = ROUTE_COLOR_FREE if free else ROUTE_COLOR
...
def show_plane(plane):
    ...
    try:
        ...
        route_label.color = ROUTE_COLOR_FREE if _route_free_tier else ROUTE_COLOR
